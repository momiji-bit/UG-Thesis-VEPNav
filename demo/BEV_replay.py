...
if __name__ == '__main__':
    # threading.Thread(target=refresh, args=("oakCam_Data_1716455258.5979753.npz",)).start()
    out = cv2.VideoWriter(f'output{time.time()}.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 30, (1080+640, 1080))
    # while True:
    #     if thread_is_running:
    #         break
    #     time.sleep(0.01)

    data = np.load("oakCam_Data_1716454357.15981.npz")
    ...
    for i in range(1, len(frames_BGR_loaded)):
        ...
        # start_x = width // 2 - 600
        # start_y = height // 2 - 600
        # end_x = start_x + 1200
        # end_y = start_y + 1200
        # BEV = BEV[start_y:end_y, start_x:end_x]


        img_mix = np.vstack((img_BGR, seg, img_D_COLORMAP))
        BEV = cv2.resize(BEV, (img_mix.shape[0], img_mix.shape[0]), interpolation=cv2.INTER_AREA)
        img_mix = np.hstack((img_mix, BEV))
