...
class MscEvalV0(object):
    ...
    def checklast(self, p1, p2):
        if self.lastslopes == []:
            ...
            if self.distance(p1[0:2], self.lastslopes) + self.dist(p_1, (1920 / 2, 1080 / 2)) / 1000 < self.distance(
                    p1[0:2], self.lastslopes) + self.dist(p_2, (1920 / 2, 1080 / 2)) / 1000:
                self.lastslopes = list(p1[0:2])
            else:
                self.lastslopes = list(p2[0:2])
            print("hof slopes:", self.distance(p1[0:2], self.lastslopes), "hof dist:",
                  self.dist(p_1, (1920 / 2, 1080 / 2)) / 1000, "str slopes:", self.distance(p2[0:2], self.lastslopes),
                  "str dist:", self.dist(p_2, (1920 / 2, 1080 / 2)) / 1000)

            if (self.distance(p1[0:2], self.lastslopes) + self.dist(p_1, (1920 / 2, 1080 / 2)) / 1000) < (
                    self.distance(p1[0:2], self.lastslopes) + self.dist(p_2, (1920 / 2, 1080 / 2)) / 1000):
                return p1 + ("(by hof)",)
            else:
                return p2 + ("(by str)",)
            # return p1+("hof",)
    ...
    def distance(self, x1, x2):
        return np.sqrt((x1[0] - x2[0]) * (x1[0] - x2[0]) + (x1[1] - x2[1]) * (x1[1] - x2[1])) + np.sqrt(
            (x1[0] - 0.7) * (x1[0] - 0.7) + (x1[1] + 0.7) * (x1[1] + 0.7)) + abs(1 / (x1[0] - 0.05)) + abs(
            1 / (x1[1] - 0.05)) + abs(1 / (x1[0] + 0.05)) + abs(1 / (x1[1] + 0.05)) + abs(1 / (x1[0] - 0.05)) + abs(
            1 / (x1[1] - x1[0]))
